...
const SidebarTablet = () => {
  ...
  return (
    <nav className={cn("flex flex-col h-[100svh] border-r border-[#E2E8F0]", isOpen ? "w-[270px]" : "w-[72px]")}>
      <header className="flex items-center gap-[10px] px-6 py-8 relative">
        <div className="flex items-center gap-1">
          ...
          {!isOpen && (
            <div className="w-7 h-7 bg-gray-400 ml-[10px] cursor-pointer" onClick={() => setIsOpen(!isOpen)} />
          )}
        </div>

        {isOpen && <div className="w-7 h-7 bg-gray-400 cursor-pointer" onClick={() => setIsOpen(!isOpen)} />}
      </header>
